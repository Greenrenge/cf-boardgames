...
1. CSS aspect-ratio for image display (no padding hacks)
2. Native browser lazy loading (no external libraries)
3. Optional fields with defaults for backward compatibility
4. Fisher-Yates shuffle for fair spy selection
5. Modulo distribution for role repetition
6. Tailwind Grid for responsive layout
7. Simple state machine for image loading
8. Extend existing WebSocket messages (no new types)

**Complexity Added**: Minimal - all standard patterns, zero new dependencies
...
- `data-model.md` - 6 core entities with validation rules and state transitions
- `contracts/websocket.md` - 6 message types (1 new, 5 extended)
- `contracts/http-api.md` - 3 API endpoints (1 new, 2 extended)
- `quickstart.md` - 5-phase implementation guide with code examples

**Entity Summary**:

- Room Configuration (extended with maxPlayers, spyCount)
- Room State (modified for capacity tracking)
- Game State (modified for multi-spy support)
- Location Image Metadata (no changes)
- Player (no changes)
- Image Display State (client-only)

**API Changes**:

- NEW: `ROOM_CONFIG_UPDATE` WebSocket message
- NEW: `PATCH /rooms/:code/config` HTTP endpoint
- Extended: `ROOM_STATE`, `YOUR_ROLE`, `GAME_STARTED`, `VOTING_RESULTS`, `SPY_GUESS_RESULT`
- Extended: `POST /rooms`, `GET /rooms/:code`

**Backward Compatibility**: Full - old clients ignore new fields, new clients handle missing fields
...
- Atomic implementation tasks (2-4 hour chunks)
- Dependency graph (which tasks block others)
- File-level changes mapped to tasks
- Validation checkpoints per user story

**Not included in this plan** - Phase 2 is a separate command as per the prompt.
...
- ✅ Implementation guide written (quickstart.md)
...
| --------------------------- | --------------- |
| P1: Non-spy image display   | 4-6 hours       |
| P2: Spy location browser    | 6-8 hours       |
| P3: Backend player capacity | 3-4 hours       |
| P4: Frontend config UI      | 4-6 hours       |
| P5: Multi-spy logic         | 4-6 hours       |
| Testing & Polish            | 4-6 hours       |
| **Total**                   | **25-36 hours** |

**No Blockers**: All dependencies available (R2 images, existing architecture, Tailwind CSS)
...
2. Create feature branch: `git checkout -b 003-location-images-scaling`
3. Follow `quickstart.md` phases sequentially
4. Test each phase via playground (spec.md validation steps)
5. Deploy to staging, verify with 10+ real players
...
```
specs/003-location-images-scaling/
├── plan.md              ← You are here (4,134 lines)
├── research.md          ← 330 lines, 8 decisions
├── data-model.md        ← 362 lines, 6 entities
├── quickstart.md        ← 602 lines, 5 phases
└── contracts/
    ├── websocket.md     ← 385 lines, 6 messages
    └── http-api.md      ← 278 lines, 3 endpoints

Total: ~6,091 lines of planning documentation
```

**All artifacts committed to git** - Ready for team review and implementation.
