...
export default function RoomPage() {
  ...
  const handleKicked = (payload: { reason: string }) => {
    ...
    setError(payload.reason || t('message.kickedByHost'));
  ...
  const handleSkipTimer = () => {
    ...
    if (currentPlayerId !== hostId) {
      setError('Only the host can skip timer');
      return;
    }
  ...
  const handleResetGame = () => {
    ...
    if (currentPlayerId !== hostId) {
      setError('Only the host can reset the game');
      return;
    }

    if (confirm(t('message.confirmResetGame'))) {
          ...
          {playerRole === 'Spy' ? (
            ...
            <Card className="p-8">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">{t('message.pleaseWait')}</p>
                <div className="mt-6">
